...
void loop() {
  while (Serial.available() > 0) {
    ...
    if (rec == 'r') {
      ...
      Serial.println(serData);
      mov_step(YSTEP, RIGHT, 5, Mspeed*4);
      serData = "";
      delay(delaytime);
    }
    else if (rec == 'D') {
      Serial.println(serData);
      mov_step(YSTEP, RIGHT, 10, Mspeed*4);
      serData = "";
      delay(delaytime);
    }
    else if (rec == 'p') {
      ...
      Serial.println(serData);
      mov_step(YSTEP, RIGHT, 800, Mspeed*4);
      serData = "";
      delay(delaytime);
    }
    else
...
void home_init() {   //Find Home position
  ...
  while (Xhome == 0) { // Home
    ...
    if (Xhome == 0) {
      if (SW == 1)
        mov_step(XSTEP, RIGHT, 1, Mspeed*10);
      else if (SW == 0) {      // SW ON
        mov_step(XSTEP, LEFT, 70, Mspeed*10);
        Xhome = 1;
      }
    }
  }
  while (Yhome == 0) {
    ...
    if (P_in == 1)
      mov_step(YSTEP, RIGHT, 1, Mspeed * 4);
    else if (P_in == 0) {    // Paper Detected
      mov_step(YSTEP, RIGHT, 190, Mspeed * 4);
      Yhome = 1;
    }
  }
}
